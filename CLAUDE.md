...
- **darrenkre.com is an alias and 301-redirects** to darrenkrealestate.com. The redirect lives **in the Worker** (`src/worker.js`) — this is the chosen and only mechanism; do **not** add a dashboard Redirect Rule for it. It's hostname-guarded so it can only fire for `darrenkre.com`/`www.darrenkre.com` — never for the canonical domain or the `workers.dev` preview, so it cannot loop. Both hostnames land on the apex `darrenkrealestate.com` (www is intentionally dropped — one canonical host).
  - **Required Cloudflare setup (dashboard only — not in this repo):** `darrenkre.com` **and** `www.darrenkre.com` must be attached to the `darrenkre` Worker under Workers &amp; Pages → darrenkre → Settings → **Domains & Routes**. Without that route the Worker never executes for those hostnames and the domain just errors (Cloudflare 1001) — which is exactly the "doesn't work, doesn't redirect" symptom.
  - **Redirect Rules run *before* Workers** in Cloudflare's pipeline. If one exists for this zone it wins and `src/worker.js` never sees the request, so keep that zone free of redirect rules to avoid a confusing conflict.
...
- `photo-archive/` — **unlisted internal report** on the property-photo recovery work (`index.html` summary + `finds.html` full 428-image inventory, with its own self-contained `photo-archive/assets/`). Reachable only by direct link at `/photo-archive/`; both pages carry `<meta name="robots" content="noindex, nofollow">` and are **deliberately not linked from any nav, footer, or `sitemap.xml`**. Rewritten (8 Aug 2026) in plain language addressed **to Darren** — print sizes ("prints about 9″ × 14″") instead of pixel counts, with the evidence tucked into a collapsed `<details class="tech">` block. It names a **third-party individual** (the original photographer, E R Lessey) and recommends contacting him, and it frankly discusses what the company lacks — **so keep it unlisted and check with Edward before linking it anywhere public-facing.** Do not add a `Disallow` for it in `robots.txt`: that would stop crawlers reading the `noindex`, which is the stronger signal. Keeps its own `assets/` dir so nothing collides with the site's.
...
- **Mortgage estimator** (`.mortgage` block, wired in `main.js`): collapsible "Estimate My Mortgage" panel with a brass donut (Principal & Interest / Property Taxes / Maintenance) + Total Monthly, and inputs for home price, down payment ($/% synced with a slider), term (30/15), interest, and monthly taxes/maintenance. **Reusable — reads the price from `data-price`.** P&I uses the standard amortization formula (verified against industry calculators). **Add this block to every listing page**, with that listing's price. **Maintenance/common charges are provided by Darren per property** (30 W 13th, 4A = **$2,979/mo**; at the current $1,995,000 ask with 20% down, 30yr @ 6.99% → P&I $10,608 + $2,979 = **$13,587/mo**); for co-ops leave Property Taxes at `$0` (the tax is bundled into maintenance). All inputs stay editable.
...
- **Open house pop-up** (`main.js` IIFE + `.oh-*` CSS): elegant site-wide modal (photo collage + event dates/CTAs), shown once per session and auto-expiring after the last event date (`OH_KEY` + `deadline` in code). Skips on the listing detail page. Buttons: **View the Listing** (→ `/30West13thStreet.html`) and **StreetEasy**. Current event: **Sat August 8, 2026 · 1:00–3:00 PM ET** and **Sun August 9, 2026 · 2:00–4:00 PM ET** — deadline `2026-08-09T20:00:00Z`, i.e. it stops appearing the moment Sunday's session ends. Update the two `.oh-date-row` lines, `OH_KEY` (bump it so people who dismissed the previous pop-up see the new one), and `deadline` for future events; remove the IIFE when there's no upcoming open house.
  - **Per-date Facebook event links** (`.oh-rsvp` in the pop-up + `.featured-link` "Open House · <date>" buttons on the listing) are added when Darren creates a new FB event per date. Current links: Sat Aug 8 = `https://www.facebook.com/share/1JeJ1GduWw/`, Sun Aug 9 = `https://www.facebook.com/share/1BWeyDWYq4/`. Darren makes **new events each time**, so always replace the URLs rather than reusing old ones.
