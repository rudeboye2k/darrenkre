...
- `30West13thStreet.html` — dedicated listing page (short URL, no hyphens; one page per listing going forward). **Every listing page must include the mortgage estimator** (`.mortgage` block with `data-price="<price>"`, no `$`/commas), the full listing description, and an elegant **Google Maps location link** (`.listing-loc` — pin + address + `maps.google.com/.../search/?api=1&query=<address>` link) — see Features.
